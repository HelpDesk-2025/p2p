/*
  # Update get_my_pending_approval_ids to include alternate approvers

  ## Summary
  Updates the server-side RPC function that determines which pending requests
  appear in an approver's queue. Now also returns requests where the current user
  is the alternate approver at the current step (not just the primary approver).

  ## Changes
  - `get_my_pending_approval_ids` function updated to check `af.alternate_approver_id = v_user_id`
    in addition to `af.user_id = v_user_id` for all 5 request types.
  - All other matching logic (Department Head, Procurement, President, etc.) unchanged.
*/
