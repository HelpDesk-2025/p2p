/*
  # Align pending-approval counts with requester filter

  ## Summary
  Mirror the fix from `get_my_pending_approval_ids` in
  `get_user_pending_approval_counts` so dashboard badge counts reflect the same
  filtered sequences (skip steps where all approvers are effectively the
  requester).
*/
