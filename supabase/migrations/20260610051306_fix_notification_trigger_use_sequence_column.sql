...
BEGIN
  ...
  IF NEW.action = 'Submitted' THEN
    ...
    END IF;
    -- Notify first approver: find via approval_flow_setups join, using af.sequence (not sequence_number)
    FOR v_next_approver IN
      SELECT DISTINCT af.user_id FROM approval_flows af
      JOIN approval_flow_setups afs ON af.approval_flow_setup_id = afs.id
      WHERE afs.request_type = NEW.request_type
        AND af.sequence = 1
        AND af.is_active = true
        AND af.user_id IS NOT NULL
        AND af.user_id IS DISTINCT FROM v_requester_id
        AND afs.company_id = NEW.company_id
      LIMIT 3
    LOOP
      INSERT INTO user_notifications (user_id, title, message, notification_type, request_type, request_number, request_id, target_view)
  ...
  ELSIF NEW.action = 'Approved' THEN
    ...
    IF v_requester_id IS NOT NULL AND v_requester_id IS DISTINCT FROM NEW.approver_id THEN
      ...
      VALUES (v_requester_id, v_title, v_message, v_notif_type, NEW.request_type, NEW.request_number, NEW.request_id, v_requester_view);
    END IF;
    -- Notify next approver in the sequence
    ...
    IF v_current_sequence IS NOT NULL THEN
      FOR v_next_approver IN
        ...
        JOIN approval_flow_setups afs ON af.approval_flow_setup_id = afs.id
        WHERE afs.request_type = NEW.request_type
          AND af.sequence = v_current_sequence + 1
          AND af.is_active = true
          AND af.user_id IS NOT NULL
          AND af.user_id IS DISTINCT FROM v_requester_id
          AND afs.company_id = NEW.company_id
