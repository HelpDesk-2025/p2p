/*
  # Fix Pending Approval Counts to Include For-Checking Steps

  1. Changes
    - Remove the `for_checking = false` filter from get_user_pending_approval_counts
    - Approvers with "For Checking Only" SHOULD see pending approvals in their dashboard
    - They can approve/reject requests, they just don't appear in signatures
    
  2. Logic
    - The for_checking flag ONLY affects signatures (excluded from RFP PDFs)
    - It does NOT affect approval workflow or pending counts
    - Approvers with for_checking=true still need to approve/reject
*/
