/*
  # Fix get_my_pending_approval_ids to use permission-based checks

  1. Changes
    - Replace hardcoded role check with permission-based check
    - Replace admin check with has_full_access from roles table
    - Keep approver_type matching using permissions instead of role names
    
  2. Security
    - Any user whose role has ANY approval permission or has full_access can be considered
    - Users explicitly assigned in approval flows are always considered
    - Full Access roles see all pending requests
*/

-- Drop both overloads first
DROP FUNCTION IF EXISTS public.get_my_pending_approval_ids(text);
DROP FUNCTION IF EXISTS public.get_my_pending_approval_ids(text, uuid);

-- Create the 2-param version
CREATE FUNCTION public.get_my_pending_approval_ids(p_request_type text, p_user_id uuid DEFAULT NULL)
...
-- Check if user has any approval-related permission (replaces hardcoded role check)
...
-- If user has no approval permissions AND is not explicitly assigned, return empty
IF NOT v_has_any_approval_permission AND NOT v_is_explicit_approver THEN RETURN; END IF;

-- Full access users see all pending requests
