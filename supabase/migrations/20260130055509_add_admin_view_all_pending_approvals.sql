/*
  # Add Admin View All Pending Approvals

  1. Changes
    - Recreate the `get_user_pending_approval_counts` function
    - For ADMIN users: Count ALL pending requests (except those at for_checking steps)
    - For REGULAR approvers: Only count requests at their current approval step
    
  2. Logic
    - If user is admin, count all pending requests where current step is not for_checking
    - If user is regular approver, use existing logic to check approval step
    - Exclude steps with `for_checking = true` from counts

  3. Security
    - Function uses SECURITY DEFINER to access all data
    - Admin sees all pending approvals
    - Regular approvers only see requests at their approval level
*/
...
BEGIN
  ...
  IF v_is_admin THEN
    ...
    LOOP
      SELECT af.for_checking INTO v_current_step
      ...
      IF v_current_step.for_checking IS NULL OR v_current_step.for_checking = false THEN
  ...
  -- Count Purchase Requisitions at user's approval step
  FOR v_request IN 
  ...
  LOOP
    ...
    IF v_current_step.id IS NOT NULL THEN
      ...
      ELSIF v_current_step.approver_type IN ('Procurement', 'Procurement Head') THEN
        v_is_current_approver := v_user_role IN ('procurement', 'approver');
      ELSIF v_current_step.approver_type = 'President' THEN
        v_is_current_approver := v_user_role = 'approver';
      END IF;
  ...
  FOR v_request IN 
    SELECT ca.id, ca.company_id, ca.department, ca.total_amount, ca.is_budgeted,
  ...
  -- Return counts for regular approvers
  RETURN QUERY SELECT v_pr_count, v_canvass_count, v_pc_count, v_reimb_count, v_ca_count;
END;
$$;
