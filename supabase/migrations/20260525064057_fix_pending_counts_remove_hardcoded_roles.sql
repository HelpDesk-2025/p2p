/*
  # Fix get_user_pending_approval_counts to use permission-based checks

  1. Changes
    - Replace hardcoded role check with permission-based check
    - Replace admin check with has_full_access check
    - Use user_has_permission for approver_type matching
    
  2. Security
    - Any user with approval permissions or full_access can see counts
    - Full Access roles see total counts across all companies
*/

DROP FUNCTION IF EXISTS public.get_user_pending_approval_counts();

CREATE FUNCTION public.get_user_pending_approval_counts()
...
-- Full access users see all counts
