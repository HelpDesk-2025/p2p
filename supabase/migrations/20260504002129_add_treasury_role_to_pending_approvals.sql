/*
  # Allow treasury role to see pending approvals

  ## Summary
  Update both pending-approval RPCs to allow users with `role = 'treasury'`
  (analogous to the existing `approver` role) so treasury users can see their
  assigned approval items on the Dashboard and in approval queues.

  ## Changes
  - `get_my_pending_approval_ids(text, uuid)`: include `treasury` in the allowed
    caller roles and in the Department Head role check.
  - `get_user_pending_approval_counts()`: same, so the dashboard badge count is
    populated for treasury users.
*/

-- Patch get_my_pending_approval_ids(text, uuid): allow treasury caller and
-- treat treasury like approver for Department Head matches.
...
-- Patch get_user_pending_approval_counts: allow treasury caller.
