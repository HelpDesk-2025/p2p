-- Fix ManCom expense PR routing: when is_mancom_expense = true,
-- route through the department's primary executive's approval steps
-- instead of the normal department flow.

CREATE OR REPLACE FUNCTION get_my_pending_approval_ids(
p_request_type text,
...
-- ManCom expense PRs: route through the department's primary executive's approval steps
IF COALESCE(v_request.is_mancom_expense, false) THEN
v_exec_is_budgeted := COALESCE(v_request.is_budgeted, false);
v_exec_category := CASE WHEN v_exec_is_budgeted THEN 'budgeted' ELSE 'non_budgeted' END;

-- Determine workflow_type to find the primary executive
SELECT president_min_amount INTO v_president_min_amount FROM companies WHERE id = v_request.company_id;
IF NOT COALESCE(v_request.is_budgeted, false) THEN v_workflow_type := 1;
ELSIF v_request.total_amount < COALESCE(v_president_min_amount, 0) THEN v_workflow_type := 2;
ELSE v_workflow_type := 3;
END IF;

-- Find the primary executive (first non-checking approver) in the normal department flow
-- This is the person whose ManCom budget the expense belongs to
SELECT af.user_id INTO v_mancom_exec_id
...
IF v_mancom_exec_id IS NOT NULL THEN
-- Check if this executive has executive_approval_steps configured
SELECT COUNT(*) INTO v_exec_step_count
FROM executive_approval_steps
WHERE user_profile_id = v_mancom_exec_id AND category = v_exec_category;

IF v_exec_step_count > 0 THEN
-- Use the executive's approval steps
SELECT email INTO v_exec_step_email
FROM executive_approval_steps
WHERE user_profile_id = v_mancom_exec_id AND category = v_exec_category
ORDER BY sequence OFFSET v_request.current_approval_level LIMIT 1;
...
END IF;
END IF;
-- If no executive steps found, fall through to normal flow
END IF;
