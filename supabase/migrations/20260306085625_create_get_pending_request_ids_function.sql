/*
  # Create function to get pending request IDs for current approver

  1. New Function
    - `get_my_pending_approval_ids` - Returns request IDs where the current user is the next approver
    - Accepts a request_type parameter to filter by type
    - Uses SECURITY DEFINER to bypass RLS and access all pending requests
    - Performs all approval flow matching server-side (eliminates N+1 client queries)

  2. Purpose
    - Replaces client-side N+1 approval flow checking that caused 20-40 second load times
    - For each pending request, determines the current approval step and checks if the user matches
    - Returns only the IDs the user needs to see, so the client can do a simple `in()` query

  3. Security
    - SECURITY DEFINER runs with creator privileges to access all tables
    - Only returns IDs for the authenticated user (auth.uid())
    - Excludes requests where the user is the requester
    - Granted to authenticated role only

  4. Matching Logic
    - Supports user_id direct match
    - Supports Department Head role-based match
    - Supports Procurement/Procurement Head role match
    - Supports Accounting/Accounting Head role match
    - Supports President role match
    - Calculates workflow_type based on budget status and amount
    - Handles executive approval flows (approver_email/checker_email on user profile)
*/
...
GRANT EXECUTE ON FUNCTION get_my_pending_approval_ids(text) TO authenticated;
