/*
  # Add Purchase Order support to get_my_pending_approval_ids RPC

  ## Summary
  The get_my_pending_approval_ids RPC previously did not handle 'Purchase Order'
  as a request type. This migration adds a new ELSIF branch for Purchase Orders,
  allowing the PO Approval page to correctly identify which POs are awaiting a
  given user's approval.

  ## Changes
  1. Added 'Purchase Order' case to the admin view-all section
  2. Added 'Purchase Order' case to the standard approver lookup section
     - Uses workflow_type = 1 (single workflow for POs)
     - Uses prepared_by as the requester equivalent
     - No executive approver logic needed for POs

  ## Security
  - SECURITY DEFINER preserved
  - No data changes; logic-only addition
*/
