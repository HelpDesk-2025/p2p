/*
  # Skip requester-only sequences in pending approvals RPCs

  ## Summary
  The frontend's `filterApprovalFlowsForRequester` removes approval flow steps
  where the only approver is the request's requester (or a Department Head
  matching the requester). The two pending-approvals RPCs
  (`get_my_pending_approval_ids`, `get_user_pending_approval_counts`) did not
  apply this same filter, so a request whose first approver is the requester
  would "stick" on that step and never appear in the next approver's queue.

  ## Changes
  Introduce a helper function `_current_sequence_for_request` that returns the
  effective current sequence (by `current_approval_level`) after excluding any
  sequences whose approvers all resolve to the requester. Both RPCs now call
  this helper instead of computing the sequence inline.

  Both RPCs are recreated to use the helper. Logic for executive requesters and
  per-request-type workflow selection is preserved.
*/
...
GRANT EXECUTE ON FUNCTION _current_sequence_for_request(uuid, text, text, integer, uuid, integer) TO authenticated;
...
BEGIN
  ...
  IF v_user_role NOT IN ('approver', 'admin', 'procurement', 'accounting') THEN RETURN; END IF;
