/*
  # Fix Pending Approval Counts to Exclude For-Checking Steps

  1. Changes
    - Recreate the `get_user_pending_approval_counts` function
    - Only count requests where:
      - User is the actual current approver
      - The current step is NOT marked as `for_checking` (those belong to Procurement Checking)
      - Request hasn't been created by the approver themselves
    
  2. Logic
    - For each request type, check if user is at the current approval step
    - Exclude steps with `for_checking = true`
    - Use proper approval flow matching
