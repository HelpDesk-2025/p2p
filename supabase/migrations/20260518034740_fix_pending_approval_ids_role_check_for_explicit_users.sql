/*
  # Fix role check in get_my_pending_approval_ids for explicitly assigned users

  ## Summary
  The RPC had an early return that blocked users whose role wasn't in the
  approved list (approver, admin, procurement, accounting, treasury). However,
  users can be explicitly assigned in approval_flows via user_id regardless of
  their role. This fix adds a check: if the user is explicitly assigned in any
  active approval flow, they bypass the role gate.

  ## Changes
  1. Modified the role check to also allow users who are explicitly assigned
     (user_id or alternate_approver_id) in any active approval_flows entry.

  ## Security
  - SECURITY DEFINER preserved
  - Users can still only see requests where they are the current approver
  - No data changes
*/
