/*
  # Add Accounting Role to Pending Approval Counts

  1. Changes
    - Update get_user_pending_approval_counts function to include accounting role
    - Add accounting role checks for all request types (PR, Canvass, Cash Advance, Petty Cash, Reimbursement)
    - Allow accounting users to see their pending approvals in the dashboard
  
  2. Security
    - Maintains existing security checks
    - Only returns counts for users with approval permissions
*/
...
BEGIN
  ...
  -- Check if user has approval permissions (added accounting role)
  ...
  LOOP
    ...
    IF v_current_sequence IS NOT NULL THEN
      -- Check if user matches ANY approval flow at this sequence (added accounting role)
