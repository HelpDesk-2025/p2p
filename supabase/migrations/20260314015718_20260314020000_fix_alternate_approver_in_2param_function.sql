/*
  # Fix alternate approver check in 2-parameter version of get_my_pending_approval_ids

  ## Summary
  The function has two overloads:
  - 1-param: get_my_pending_approval_ids(text) — used without impersonation
  - 2-param: get_my_pending_approval_ids(text, uuid) — used by the frontend (passes p_user_id)

  The 1-param version was updated to include alternate_approver_id checks.
  The 2-param version (which the frontend actually calls) was missing those checks,
  so alternate approvers (OR approvers) could not see pending requests in their queue.

  ## Changes
  - Added `(af.alternate_approver_id = v_user_id) OR` to all 5 request type blocks
    in the 2-parameter version of the function.
*/
