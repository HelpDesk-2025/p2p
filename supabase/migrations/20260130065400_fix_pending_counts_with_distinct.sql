/*
  # Fix Pending Approval Counts - Use DISTINCT to Avoid Duplicates

  1. Changes
    - Update `get_user_pending_approval_counts` function
    - Use DISTINCT when selecting current approval steps to avoid counting duplicates
    - Prevents the same request from being counted multiple times due to duplicate approval_flows entries
    
  2. Logic
    - For each request type, use DISTINCT ON to get only one approval step per setup/sequence combination
    - This ensures each pending request is only counted once
    
  3. Security
    - Function uses SECURITY DEFINER to access all data
    - Only returns counts for requests at user's approval level
*/
...
BEGIN
  ...
  LOOP
    -- Get the current approval step (use DISTINCT to avoid duplicates)
    SELECT DISTINCT ON (af.approval_flow_setup_id, af.sequence) af.* 
    INTO v_current_step
    ...
    AND af.for_checking = false  -- Exclude checking steps
    ORDER BY af.approval_flow_setup_id, af.sequence, af.id
  ...
  -- Count Canvass Requests (similar logic)
  FOR v_request IN 
