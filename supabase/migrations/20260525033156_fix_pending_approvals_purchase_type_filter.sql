/*
  # Fix pending approvals not showing for approvers after budget status change

  1. Changes
    - Update `_current_sequence_for_request` to accept optional `p_purchase_type` parameter
    - Filter out approval flow steps that don't apply to the request's purchase type
    - Update `get_my_pending_approval_ids` to pass purchase_type for Purchase Requisitions and Canvass

  2. Problem
    - When budget status changes from Budgeted to Non-Budgeted, the workflow type changes
    - Some steps in the new workflow have `applies_to_non_po = false` (e.g., PO-only checkers)
    - The RPC function was not filtering these out, causing a mismatch with the frontend
    - Result: the approver for the actual next step could not see the request in their pending list

  3. Fix
    - Add purchase type filtering in `_current_sequence_for_request`
    - Pass purchase_type from the request tables into the helper function
*/

-- Drop and recreate _current_sequence_for_request with purchase_type parameter
CREATE OR REPLACE FUNCTION _current_sequence_for_request(
  p_company_id uuid,
  p_department text,
  p_request_type text,
...
v_requester_department text;
v_sequence integer;
BEGIN
SELECT role, department INTO v_requester_role, v_requester_department
FROM user_profiles WHERE id = p_requester_id;

SELECT seq INTO v_sequence
FROM (
SELECT DISTINCT af.sequence AS seq
FROM approval_flows af
WHERE af.approval_flow_setup_id IN (
SELECT afs.id FROM approval_flow_setups afs
WHERE afs.company_id = p_company_id
AND (afs.department_id = p_department OR (afs.department_id IS NULL AND NOT EXISTS (
SELECT 1 FROM approval_flow_setups afs2
WHERE afs2.company_id = p_company_id
AND afs2.department_id = p_department
...
AND af.workflow_type = p_workflow_type
-- Filter by purchase type applicability
AND (
  p_purchase_type IS NULL
  OR (p_purchase_type = 'Purchase Order' AND af.applies_to_po IS NOT FALSE)
  OR (p_purchase_type = 'Non-Purchase Order' AND af.applies_to_non_po IS NOT FALSE)
  OR (p_purchase_type NOT IN ('Purchase Order', 'Non-Purchase Order'))
)
AND EXISTS (
SELECT 1 FROM approval_flows af2
WHERE af2.approval_flow_setup_id = af.approval_flow_setup_id
AND af2.workflow_type = af.workflow_type
AND af2.sequence = af.sequence
AND af2.is_active = true
AND NOT (
(af2.user_id = p_requester_id
AND (af2.alternate_approver_id IS NULL OR af2.alternate_approver_id = p_requester_id))
OR (af2.approver_type = 'Department Head'
AND v_requester_role = 'approver'
AND v_requester_department = p_department)
)
)
ORDER BY seq
OFFSET p_current_level
LIMIT 1
) s;
...
-- Now recreate get_my_pending_approval_ids (2-param version) to pass purchase_type
...
-- Check if user is explicitly assigned in any approval flow
...
IF v_user_role NOT IN ('approver', 'admin', 'procurement', 'accounting', 'treasury') AND NOT v_is_explicit_approver THEN RETURN; END IF;
...
-- Also update the 1-param version to use the new function signature
CREATE OR REPLACE FUNCTION get_my_pending_approval_ids(p_request_type text)
RETURNS TABLE(request_id uuid)
LANGUAGE plpgsql SECURITY DEFINER
AS $$
BEGIN
RETURN QUERY SELECT * FROM get_my_pending_approval_ids(p_request_type, NULL::uuid);
END;
$$;