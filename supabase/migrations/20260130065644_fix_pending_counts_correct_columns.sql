/*
  ...
  1. Changes
    ...
    - Fix column names for cash_advance, petty_cash, and reimbursement tables
    - Use DISTINCT ON correctly to avoid counting duplicates
    
  2. Column Names by Table
    - purchase_requisitions, canvass_requests: total_amount, is_budgeted
    - cash_advance_requests: amount, budgeted
    - petty_cash_requests: amount, budgeted (but we don't actually need these for counting)
    - reimbursement_requests: amount, budgeted (but we don't actually need these for counting)
*/
...
BEGIN
  ...
  LOOP
    -- Get the current approval step (use LIMIT to get just one row even if duplicates exist)
