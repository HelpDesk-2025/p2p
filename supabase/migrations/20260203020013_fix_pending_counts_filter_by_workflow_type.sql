/*
  # Fix Pending Approval Counts - Filter by Workflow Type

  1. Problem
    - RPC function was not filtering approval flows by workflow_type
    - It was matching all approval flows regardless of budget status
    - Example: Treasury PRs (unbudgeted, workflow_type=1) were matching Jodell at workflow_type=2
    
  2. Solution
    - Calculate workflow_type for each request based on is_budgeted and total_amount
    - Filter approval flows to only match the correct workflow_type
*/
...
BEGIN
  ...
  LOOP
    -- Get president min amount for workflow type calculation
    ...
    IF NOT v_request.is_budgeted THEN
      v_workflow_type := 1; -- UNBUDGETED
    ELSIF v_request.total_amount < v_president_min_amount THEN
      v_workflow_type := 2; -- BUDGETED_BELOW_MIN
    ELSE
      v_workflow_type := 3; -- BUDGETED_ABOVE_MIN
    END IF;
    
    -- Get the UNIQUE sequence number at current approval level (filtered by workflow_type)
    ...
    IF v_current_sequence IS NOT NULL THEN
      -- Check if user matches ANY approval flow at this sequence (filtered by workflow_type)
  ...
  -- Count Petty Cash Requests (use workflow_type 1 for non-budget requests)
  FOR v_request IN 
    SELECT pc.id, pc.company_id, pc.department, pc.current_approval_level, pc.requester_id
    FROM petty_cash_requests pc
    WHERE pc.status = 'pending'
  LOOP
    v_workflow_type := 1; -- Petty Cash always uses workflow type 1
  ...
  -- Count Reimbursement Requests (use workflow_type 1 for non-budget requests)
  FOR v_request IN 
    SELECT rr.id, rr.company_id, rr.department, rr.current_approval_level, rr.requester_id
    FROM reimbursement_requests rr
    WHERE rr.status = 'pending'
  LOOP
    v_workflow_type := 1; -- Reimbursement always uses workflow type 1
