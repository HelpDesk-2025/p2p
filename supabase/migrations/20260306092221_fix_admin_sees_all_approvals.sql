/*
  # Fix admin role to see all approvals

  1. Changes
    - Updated `get_my_pending_approval_ids` so that when the caller is an admin
      and either no `p_user_id` is provided OR `p_user_id` matches the admin's own ID,
      all pending requests are returned
    - Only when an admin explicitly passes a DIFFERENT user's ID (impersonation)
      does the function filter for that specific user

  2. Purpose
    - Admin users should always see all pending approvals when viewing as themselves
    - The previous fix for impersonation inadvertently made admin pass their own ID,
      which caused the function to skip the "return all" admin branch

  3. Security
    - Only authenticated admin users get the "see all" behavior
    - Non-admin users always see only their own assigned approvals
    - Impersonation (admin passing a different user's ID) correctly filters for that user
*/

DROP FUNCTION IF EXISTS get_my_pending_approval_ids(text);
