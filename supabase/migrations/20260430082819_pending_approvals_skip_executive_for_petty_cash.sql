/*
  # Skip executive short-circuit for Petty Cash in pending approvals

  ## Summary
  The `get_my_pending_approval_ids` RPC short-circuited Petty Cash requests
  whose requester has `approver_type = 'Executive'` to the executive flow
  (using the requester's `approver_email`/`checker_email`). The frontend
  (ApprovalProgressTracker and Petty Cash request submission) intentionally
  bypasses the executive override for Petty Cash and uses the configured
  Approval Flow Setup as the source of truth, driven by Expense Category.

  This mismatch caused Petty Cash requests from executive requesters to never
  appear in the actual Petty Cash approver's queue. This migration drops the
  executive short-circuit for the Petty Cash branch so behavior matches the
  frontend: the configured Approval Flow Setup is always used.
*/
...
BEGIN
  ...
  ELSIF p_request_type = 'Petty Cash' THEN
    -- Petty Cash uses the configured Approval Flow Setup as the single source
    -- of truth (Expense Category drives workflow_type). No executive override.
    FOR v_request IN
