...
export function CanvassApproval() {
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      if (rpcError) {
        console.error('RPC error loading Canvass approval IDs:', rpcError);
        setRequests([]);
      ...
      console.error('Error loading canvass approvals:', error);
      setRequests([]);
    }
    setListLoading(false);
  };
          ...
          <div className="overflow-auto flex-1">
            <table className="w-full hidden lg:table">
              <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
                <tr>
                  {['CANVASS NO.', 'PR DOC NO.', 'REQUESTER', 'DEPARTMENT', 'DATE', 'AMOUNT', 'LEVEL', 'ACTION'].map((h) => (
