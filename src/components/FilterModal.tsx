...
export interface FilterColumn {
  key: string;
  label: string;
  type: 'text' | 'select' | 'multiSelect' | 'date' | 'number' | 'dateRange' | 'checkbox';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col animate-in fade-in zoom-in-95">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {columns.filter((col) => !col.showWhen || col.showWhen(localValues)).map((col) => (
            <div key={col.key}>
              {col.type !== 'checkbox' && (
              ...
              {col.type === 'checkbox' && (
                <label className="flex items-start gap-2.5 px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-100 transition-colors">
                  <input
                    type="checkbox"
                    checked={localValues[col.key] === 'true'}
                    onChange={(e) => handleChange(col.key, e.target.checked ? 'true' : '')}
