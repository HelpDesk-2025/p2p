          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl my-4 sm:my-8 max-h-[95vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white rounded-t-xl sm:rounded-t-2xl">
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
                  {editingId ? 'Update the checklist item details and attachments' : 'Create a new PR checklist item'}
                </p>
              </div>
              <button
                onClick={handleCancel}
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl my-4 sm:my-8 max-h-[95vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white rounded-t-xl sm:rounded-t-2xl">
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
                  {editingId ? 'Update payment mode details and line names' : 'Create a new payment mode'}
                </p>
              </div>
              <button
                onClick={handleCancel}
...
function ExpenseTypesConfig({ data, reload }: { data: any[]; reload: () => void }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-slate-900">Type of Expense</h2>
        <button
          onClick={() => setShowForm(true)}
          className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
        >
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl my-4 sm:my-8 max-h-[95vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white rounded-t-xl sm:rounded-t-2xl">
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
                  {editingId ? 'Update expense type details and sub-items' : 'Create a new expense type'}
                </p>
              </div>
              <button
                onClick={handleCancel}
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-200">
            {data.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50">
                <td className="px-6 py-4 text-sm text-slate-900 font-medium">{item.name}</td>
                <td className="px-6 py-4 text-sm text-slate-900">{parseFloat(item.rate).toFixed(2)}%</td>
                <td className="px-6 py-4 text-sm text-slate-600">{item.description || '-'}</td>
                <td className="px-6 py-4 text-sm">
                  <span className={`px-2 py-1 rounded-full text-xs ${item.is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
                    {item.is_active ? 'Active' : 'Inactive'}
