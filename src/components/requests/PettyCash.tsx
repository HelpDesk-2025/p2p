...
export function PettyCash() {
  ...
  useEffect(() => {
    const fetchLinkedPettyCash = async () => {
      if (viewingRequest?.request_type === 'For Liquidation' && viewingRequest?.linked_petty_cash_id) {
        try {
  ...
  const handleReceivePettyCash = async (request: PettyCashReq) => {
    const liquidationOver = request.request_type === 'For Liquidation'
      && (Number(request.amount) - (Number(request.petty_cash_advance) || 0)) > 0;
    ...
    if (
      ...
      alert('Receive cash functionality is only available for "For Reimbursement", "For Cash Advance", or "For Liquidation" requests with an Over for Reimbursement amount.');
      return;
    }
  ...
  const regenerateLiquidationForm = async () => {
    ...
    try {
      ...
      // Build approvers list from all approval records (exclude for_checking)
      const approvers = ledgerData
      ...
      // Include for_checking records as "Noted by"
      const notedBy = ledgerData
        .filter((r: any) => r.for_checking)
        .map((r: any) => ({
          name: r.approver_name,
          esig: r.approver_esig || null,
      ...
      const allApprovers = [...notedBy, ...approvers];

      // Fallback for single approvedBy fields (backward compat)
          ...
          <table className="w-full border-collapse">
            <thead className="sticky top-0 bg-gradient-to-r from-slate-50 to-slate-100 border-b-2 border-slate-200 z-10">
              <tr>
                ...
                <th className="px-3 xl:px-4 py-3.5 text-left whitespace-nowrap">
                  <button onClick={() => handleSort('amount')} className="flex items-center gap-1.5 text-xs font-bold text-slate-700 uppercase tracking-wider hover:text-slate-900 transition-colors">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              {viewingRequest.status === 'approved' && viewingRequest.received_at && (viewingRequest.request_type === 'For Reimbursement' || viewingRequest.request_type === 'For Cash Advance' || viewingRequest.request_type === 'For Liquidation') && (
              ...
              {viewingRequest.request_type === 'For Liquidation' && linkedPettyCashDetails && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-900 font-bold text-green-700">
                          {formatCurrency(linkedPettyCashDetails.amount)}
                        </p>
                      </div>
                      <div>
                        <label className="text-xs font-medium text-slate-600">Status</label>
                        <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(linkedPettyCashDetails.status)}`}>
            ...
            <div className="border-t border-slate-200 px-6 py-4 bg-slate-50 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                {viewingRequest.status === 'approved' && viewingRequest.received_at && viewingRequest.approved_petty_cash_pdf_path && (viewingRequest.request_type === 'For Reimbursement' || viewingRequest.request_type === 'For Cash Advance' || viewingRequest.request_type === 'For Liquidation') && (
                  <>
                    <button
