...
export function CashAdvance() {
  ...
  const caFilterColumns: FilterColumn[] = [
    { key: 'ca_number', label: 'CA No.', type: 'text' },
  ...
  const caGetFieldValue = (item: any, key: string) => {
  ...
  const filteredRequests = applyFilters(
    requests.filter((req) => {
      ...
      return (
        req.ca_number?.toLowerCase().includes(term) ||
  ...
  const handleExport = async (exportVals: FilterValues) => {
    ...
    try {
      ...
      if (exportVals.ca_number) filtered = filtered.filter((r: any) => (r.ca_number || '').toLowerCase().includes(exportVals.ca_number.toLowerCase()));
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
          <h2 className="font-display text-xl sm:text-2xl font-semibold text-slate-900">Cash Advance Requests</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search by CA number, company, purpose, amount, status..."
              ...
              paginatedRequests.map((req, index) => (
                ...
                >
                  <td className="px-3 xl:px-4 py-3 whitespace-nowrap">
                    <span className="font-mono font-bold text-sm text-blue-900 truncate block min-w-[120px]" title={req.ca_number}>
                      {req.ca_number}
                    </span>
                  </td>
                  <td className="px-3 xl:px-4 py-3 whitespace-nowrap">
